<#
.SYNOPSIS
  Hermes 一键安装脚本 (Windows)
.DESCRIPTION
  自动检测并准备 Node.js v22+、Git、npm 镜像和 pnpm，然后安装并配置 Hermes。
.PARAMETER Version
  保留兼容参数；Hermes 安装版本由官方安装脚本决定。
.NOTES
  用法:
    powershell -ExecutionPolicy Bypass -File install-Hermes.ps1
  在线一键安装（推荐，直接在当前窗口执行，安装后立即可用）:
    irm https://你的域名/install-Hermes.ps1 | iex
  如果中文乱码，改用:
    & {$w=New-Object Net.WebClient;$w.Encoding=[Text.Encoding]::UTF8;iex $w.DownloadString('https://你的域名/install-Hermes.ps1')}
#>
param(
    [string]$Version = ""
)
...
$script:HermesVersion = if ($Version) { $Version } elseif ($env:Hermes_VERSION) { $env:Hermes_VERSION } else { "" }
...
function Ensure-NodePriority {
    ...
    try {
        ...
        try {
            ...
            Write-Info "正在修补 Hermes 启动脚本以使用正确的 Node.js 版本..."
            Patch-HermesShim -NodeV22Dir $NodeV22Dir
            return
        }
    }
...
function Patch-HermesShim {
    ...
    try {
        ...
        if ($content -like "*$nodeExe*") {
            Write-Ok "Hermes 启动脚本已使用正确的 Node.js 路径"
            return
        }
        ...
        if ($patched -ne $content) {
            ...
            Write-Ok "已修补 Hermes 启动脚本 → $nodeExe"
        } else {
            Write-Warn "未能自动修补，Hermes.cmd 格式不符合预期"
...
function Find-HermesBinary {
    ...
    # where.exe 查找
    try {
        $whereResult = & where.exe Hermes 2>$null
        if ($whereResult) {
    ...
    foreach ($dir in $searchDirs) {
        foreach ($name in @("Hermes.cmd", "Hermes.exe", "Hermes.ps1")) {
...
function Install-NodeDirect {
    ...
    $tmpPath = Join-Path $env:TEMP "Hermes-install"
...
function Run-PnpmInstall {
    ...
    try {
        ...
        $pkgSpec = if ($script:HermesVersion) { "Hermes@$($script:HermesVersion)" } else { "Hermes@latest" }
...
function Step-InstallHermes {
    Write-Step "步骤 5/7: 安装 Hermes"

    $gitVer = Get-GitVersion
    if (-not $gitVer) {
        Write-Err "Git 不可用，Hermes 的依赖需要 Git 来解析"
        Write-Host "  请先安装 Git: https://git-scm.com/downloads" -ForegroundColor Yellow
        return $false
    }

    if ($script:HermesVersion) {
        Write-Info "正在安装 Hermes v$($script:HermesVersion)，请耐心等待..."
    } else {
        Write-Info "正在安装 Hermes 最新版，请耐心等待..."
    }
    ...
    function On-InstallSuccess {
        ...
        if ($found) {
            ...
            Write-Info "Hermes 安装位置: $($found.Dir)"
        } else {
            try {
                $pnpmBin = (& $pnpmCmd bin -g 2>$null).Trim()
    ...
    # 非 GitHub 网络问题的通用错误
    Clear-GitConfigEnv
    Write-Err "Hermes 安装失败 (exit code: $($result.ExitCode))"
    if ($result.Stderr) {
        Write-Err "错误信息:"
...
function Step-Verify {
    ...
    if ($found) {
        ...
        Write-Info "Hermes 安装位置: $binDir"

        $ver = $null
        try { $ver = (& $found.Path -v 2>$null).Trim() } catch {}
        if ($ver) {
            Write-Ok "Hermes $ver 安装成功！"
            Write-Host "`n   恭喜！你的Hermes已就位！`n" -ForegroundColor Green
            return $true
        }
    }
    ...
    try {
        ...
        if ($pnpmBin -and (Test-Path $pnpmBin)) {
            ...
            $HermesCmd = Join-Path $pnpmBin "Hermes.cmd"
            if (Test-Path $HermesCmd) {
                $ver = $null
                try { $ver = (& $HermesCmd -v 2>$null).Trim() } catch {}
                if ($ver) {
                    Write-Ok "Hermes $ver 安装成功！"
                    Write-Host "`n   恭喜！你的Hermes已就位！`n" -ForegroundColor Green
                    return $true
...
function Step-Onboard {
    ...
    if ($choice -eq "0") {
        Write-Info "已跳过配置"
        Write-Info "重新执行本安装脚本或运行 Hermes onboard 即可进入配置"
        return $true
    }
    ...
    Write-Info "正在配置 Hermes..."
    $savedEAP = $ErrorActionPreference
    $ErrorActionPreference = "SilentlyContinue"
    & $HermesCmd @onboardArgs *>$null
    $onboardExit = $LASTEXITCODE
    $ErrorActionPreference = $savedEAP

    $configFile = Join-Path $env:USERPROFILE ".Hermes\Hermes.json"
    if (Test-Path $configFile) {
        Write-Ok "Hermes 配置完成！"
    } else {
        ...
        if ($models -and $models.Count -gt 0) {
            ...
            if ($modelChoice -ne "0" -and $modelChoice) {
                ...
                if ($idx -ge 0 -and $idx -lt $models.Count) {
                    ...
                    try {
                        & $HermesCmd models set $selectedModel.Id 2>$null | Out-Null
                        Write-Ok "默认模型已设置为 $($selectedModel.Label)"
                    } catch {
                        Write-Warn "模型设置未成功，可稍后通过 Hermes models set 手动设置"
                    }
                }
            }
        }
    ...
    Write-Host ""
    Write-Info "正在打开 Hermes 控制面板..."
    try { & $HermesCmd dashboard 2>$null } catch { Start-Process "http://127.0.0.1:18789" }
...
function Main {
    Write-Host ""
    Write-Host "   Hermes 一键安装脚本" -ForegroundColor Green
    ...
    if (-not $existingVer) {
        try { $existingVer = (& Hermes -v 2>$null).Trim() } catch {}
    }
    if ($existingVer -and -not $script:HermesVersion) {
        if ($found) { Add-ToUserPath $found.Dir }
        Ensure-ExecutionPolicy
        Write-Ok "Hermes $existingVer 已安装，无需重复安装"
        Write-Host "`n   你的Hermes已就位！`n" -ForegroundColor Green
        $reconfig = (Read-Host "  是否要重新配置 Hermes? [y/N]").Trim()
    ...
    if (-not (Step-InstallHermes)) { Write-Host "`n按任意键退出..."; $null = $Host.UI.RawUI.ReadKey("NoEcho,IncludeKeyDown"); return }
...
function Step-InstallHermes {
    Write-Step "步骤 5/6: 安装 Hermes"

    $installerUrl = "https://res1.Hermesagent.org.cn/install.ps1"
    $installerPath = Join-Path $env:TEMP "install-Hermes.ps1"

    try {
        Write-Info "正在下载安装脚本: $installerUrl"
        Invoke-WebRequest -Uri $installerUrl -OutFile $installerPath -UseBasicParsing

        Write-Info "正在安装 Hermes（本步骤跳过配置，下一步单独配置）..."
        & powershell.exe -NoProfile -ExecutionPolicy Bypass -File $installerPath -SkipSetup
        if ($LASTEXITCODE -ne 0) {
            Write-Err "Hermes 安装脚本执行失败 (exit code: $LASTEXITCODE)"
            return $false
        }
...
function Step-ConfigureHermes {
    Write-Step "步骤 6/6: 配置 Hermes"
    ...
    try {
        if (Test-Path $venvPython) {
            ...
            Write-Err "Hermes 配置失败 (exit code: $exitCode)"
            return $false
        }
        ...
        if (Test-Path $HermesExe) {
            ...
            Write-Err "Hermes 配置失败 (exit code: $LASTEXITCODE)"
            return $false
        }

        Write-Info "正在通过 PATH 查找 Hermes 命令..."
        & Hermes setup
        if ($LASTEXITCODE -eq 0) {
            Write-Ok "Hermes 配置完成"
            return $true
...
function Main-Hermes {
    ...
    if (-not (Step-InstallPnpm))     { Write-Host "`n按任意键退出..."; $null = $Host.UI.RawUI.ReadKey("NoEcho,IncludeKeyDown"); return }
    if (-not (Step-InstallHermes))   { Write-Host "`n按任意键退出..."; $null = $Host.UI.RawUI.ReadKey("NoEcho,IncludeKeyDown"); return }
    if (-not (Step-ConfigureHermes)) { Write-Host "`n按任意键退出..."; $null = $Host.UI.RawUI.ReadKey("NoEcho,IncludeKeyDown"); return }

    Write-Host ""
    Write-Host "  ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━" -ForegroundColor Green
    Write-Host "  Hermes 安装与配置完成！" -ForegroundColor Green
    Write-Host "  ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━" -ForegroundColor Green
    Write-Host ""
}
...
# 安装完成后刷新当前进程的 PATH，让 Hermes 立即可用
$machinePath = [Environment]::GetEnvironmentVariable("PATH", "Machine")
$userPath = [Environment]::GetEnvironmentVariable("PATH", "User")
